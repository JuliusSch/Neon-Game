...
public class SecondaryAttack extends BehaviourNode {

	public SecondaryAttack(AIBlackboard bb, Mob mob) {
		super(bb, mob);
	}
	...
	public void update() {
		...
		if (currentState == NodeState.RUNNING && timer > mob.weapon.getWeaponAttackBuildup()) {
			if (mob.weapon instanceof MultiAttackWeapon) {
				((MultiAttackWeapon) mob.weapon).secondaryAttack(mob.getMidX(), mob.getMidY(), blackboard.getPlayerDirection(mob));
